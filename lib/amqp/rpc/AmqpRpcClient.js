(function() {
  var AmqpRpcClient, DeclarationManager, GzipEncoding, MessageSerialization, Promise, Request, ResponseCode, TimeoutError, bluebird,
    __slice = [].slice;
  ...
  module.exports = AmqpRpcClient = (function() {
    function AmqpRpcClient(channel, timeout, declarationManager, serialization, logger, encoding) {
      ...
      this.encoding = encoding != null ? encoding : new GzipEncoding();
    ...
    AmqpRpcClient.prototype._send = function(request, id) {
      var promise, serializedRequest, timeout;
      serializedRequest = this.serialization.serializeRequest(request);
      ...
      bluebird.join(this.declarationManager.responseQueue(), this.declarationManager.requestQueue(request.name), this.declarationManager.exchange(), this.encoding.encode(null, serializedRequest), (function(_this) {
        return function(responseQueue, requestQueue, exchange, _arg) {
          var encoding, payload, properties;
          payload = _arg[0], encoding = _arg[1];
          properties = {
          ...
          return _this.channel.publish(exchange, request.name, payload, properties);
        };
    ...
    AmqpRpcClient.prototype._recv = function(message) {
      ...
      return new Promise((function(_this) {
        return function(resolve, reject) {
          if (message.content_encoding == null) {
            return resolve(_this._deserialize(id, message.content));
          }
          return _this.encoding.decode(message.content_encoding, message.content).then(function(content) {
            return resolve(_this._deserialize(id, content));
          });
        };
      })(this));
    };

    AmqpRpcClient.prototype._deserialize = function(id, content) {
      var e, response;
      try {
        response = this.serialization.unserializeResponse(content);
